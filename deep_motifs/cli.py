...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Deep-MOTIFs v4 - shared-base supervised/PU/rank heads with "
            "empirical responsibility-weighted PU and rank-space fusion."
        )
    )
    p.add_argument("--project-root",  type=str, required=True)
    p.add_argument("--labels-dir",    type=str, default=None)
    p.add_argument("--output-dir",    type=str, default="deep_motifs_v4_outputs")
    ...
    # Loss weights  (v4: supervised BCE has its own head; PU logit is not forced by BCE)
    p.add_argument("--w-sup",   type=float, default=0.0)
    ...
    p.add_argument("--ppr-alpha", type=float, default=1.0,
    ...
    p.add_argument("--pu-class-prior", type=float, default=0.06,
                   help="Fixed PU class prior pi for nnPU/pretrain. Default=0.06 from sensitivity analysis; use <=0 for automatic fold-local estimate.")
    p.add_argument("--prior-guided-pu", dest="prior_guided_pu", action="store_true", default=True,
                   help="Use fold-local empirical prior as Bayesian responsibility weights in nnPU.")
    p.add_argument("--no-prior-guided-pu", dest="prior_guided_pu", action="store_false",
                   help="Disable EBR-PU and use standard nnPU.")
    p.add_argument("--prior-guided-calibration", type=str, default="rank",
                   choices=["rank", "minmax", "raw"],
                   help="Calibration from empirical prior score to hidden-positive responsibility rho.")
    p.add_argument("--prior-weight-floor", type=float, default=0.10,
                   help="Minimum reliable-negative weight 1-rho for unlabeled genes.")
    p.add_argument("--prior-uncertainty-delta", type=float, default=0.05,
                   help="Entropy-shrinkage coefficient for lower-bound negative "
                        "responsibility: w=max(floor, 1-rho-delta*H(rho)/log2). "
                        "0.0 recovers the empirical-Bayes point estimate.")

    # Delta lower-bound sensitivity experiment
    p.add_argument("--delta-sensitivity", action="store_true", default=False,
                   help="Run a grid of prior-uncertainty-delta values and write each "
                        "run under --delta-sensitivity-dir.")
    p.add_argument("--delta-grid", type=str, default="0,0.025,0.05,0.075,0.10,0.15",
                   help="Comma-separated delta values for --delta-sensitivity.")
    p.add_argument("--delta-sensitivity-dir", type=str, default="sensitive_delta_lower_bound",
                   help="Directory, relative to project root unless absolute, for "
                        "delta sensitivity outputs.")
...
def parse_delta_grid(text: str) -> list[float]:
    ...
    for raw in str(text).split(","):
        ...
        val = float(raw)
        if val < 0:
            raise ValueError(f"Delta values must be non-negative, got {val}")
        vals.append(val)
    if not vals:
        raise ValueError("--delta-grid must contain at least one value.")
...
def format_delta_name(delta: float) -> str:
    token = f"{float(delta):.4f}".rstrip("0").rstrip(".")
    if token == "":
        token = "0"
    return "delta_" + token.replace("-", "m").replace(".", "p")


def collect_delta_sensitivity_summary(root: Path, deltas: list[float]) -> None:
    rows: list[dict[str, float | str]] = []
    metrics = [
        "accuracy", "precision", "recall", "f1", "pr_auc", "roc_auc",
        "precision@10", "precision@20", "precision@50", "ndcg@50",
    ]
    for delta in deltas:
        run_dir = root / format_delta_name(delta)
        for summary_name, scope in [
            ("cv_metrics_summary.csv", "fold_threshold"),
            ("cv_metrics_summary_global_threshold.csv", "global_threshold"),
        ]:
            ...
            for metric in metrics:
                ...
                if not sub.empty:
                    ...
                    row[f"{metric}_std"] = float(sub["std"].iloc[0])
            rows.append(row)

    if rows:
        out = pd.DataFrame(rows).sort_values(["threshold_scope", "delta"])
        out.to_csv(root / "delta_sensitivity_summary.csv", index=False)
        compact = out[
            [
                "delta", "threshold_scope",
                "f1_mean", "pr_auc_mean", "roc_auc_mean",
                "precision@20_mean", "precision@50_mean", "ndcg@50_mean",
                "run_dir",
            ]
        ]
        compact.to_csv(root / "delta_sensitivity_summary_compact.csv", index=False)
...
def main() -> None:
    ...
    if args.fusion_mode == "rrf":
        fusion_info = f"rrf(k={args.rrf_k})"
    elif args.fusion_mode == "pu":
        fusion_info = "pu-only(no final prior fusion)"
    else:
        fusion_info = "fixed(alpha=0.5)"
    print(
        f"[INFO] Deep-MOTIFs v4 start - device={device}  "
        ...
        f"prior_model={args.prior_model}  "
        f"w_sup={args.w_sup}"
    )

    if args.delta_sensitivity:
        deltas = parse_delta_grid(args.delta_grid)
        sensitivity_root_arg = Path(args.delta_sensitivity_dir)
        sensitivity_root = (
            sensitivity_root_arg
            if sensitivity_root_arg.is_absolute()
            else project_root / sensitivity_root_arg
        )
        sensitivity_root.mkdir(parents=True, exist_ok=True)
        pd.DataFrame({"delta": deltas}).to_csv(
            sensitivity_root / "delta_grid.csv", index=False
        )
        print(
            f"[INFO] Running lower-bound delta sensitivity: {deltas} -> "
            f"{sensitivity_root}"
        )

        for delta in deltas:
            run_output_dir = sensitivity_root / format_delta_name(delta)
            print(
                f"[INFO] Delta sensitivity run: delta={delta:.4f}  "
        ...
        label_budget_neg_ratio=args.label_budget_neg_ratio,
                ...
                pu_class_prior=args.pu_class_prior if args.pu_class_prior > 0 else None,
                prior_guided_pu=args.prior_guided_pu,
                prior_guided_calibration=args.prior_guided_calibration,
                prior_weight_floor=args.prior_weight_floor,
                prior_uncertainty_delta=float(delta),
            ...
            print(f"[DONE] Delta {delta:.4f} saved to: {run_output_dir}")

        collect_delta_sensitivity_summary(sensitivity_root, deltas)
        print(
            f"[DONE] Delta sensitivity summary saved to: "
            f"{sensitivity_root / 'delta_sensitivity_summary_compact.csv'}"
        )
        return
    ...
    run_pu(
        ...
        prior_guided_calibration=args.prior_guided_calibration,
        prior_weight_floor=args.prior_weight_floor,
        prior_uncertainty_delta=args.prior_uncertainty_delta,
        ckpt_avg_k=args.ckpt_avg_k,
        fusion_mode=args.fusion_mode,
        rrf_k=args.rrf_k,
        ppr_alpha=args.ppr_alpha,
    ...
    print(f"[DONE] Deep-MOTIFs v4 results saved to: {output_dir}")
