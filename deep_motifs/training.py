...
from .features import repeat_array
from .graph import _gcn_aggregate_string
from .graph import build_neighbor_matrix
from .models import DeepMOTIFs
from .pretrain import pretrain_encoder
from .augmentation import make_corrupted_view
from .losses import weighted_nnpu_loss
from .losses import weighted_pairwise_ranking_loss
from .losses import rank_percentile_scores
...
from .calibration import recall_at_k_score
...
def fit_deep_motifs_and_export(
    ...
    prior_guided_active = bool(prior_guided_pu and xgb_oof_scores is not None)
    prior_resp_all = np.zeros(len(ids_all), dtype=np.float32)
    prior_neg_weight_all = np.ones(len(ids_all), dtype=np.float32)
    prior_weight_floor = float(np.clip(prior_weight_floor, 0.0, 1.0))
    prior_uncertainty_delta = float(max(prior_uncertainty_delta, 0.0))

    if prior_guided_active:
        prior_series = xgb_oof_scores.reindex(ids_all).astype(float).fillna(0.5)
        universe_prior = prior_series.reindex(universe_ids).astype(float).fillna(0.5)
        if prior_guided_calibration == "rank":
            # Rank-calibrated responsibility only assumes monotone evidence order.
            rho_universe = universe_prior.rank(pct=True, method="average")
        elif prior_guided_calibration == "minmax":
            lo = float(universe_prior.min())
            hi = float(universe_prior.max())
            if hi > lo:
                rho_universe = (universe_prior - lo) / (hi - lo)
            else:
                rho_universe = pd.Series(0.5, index=universe_prior.index)
        elif prior_guided_calibration == "raw":
            rho_universe = universe_prior.clip(0.0, 1.0)
        else:
            raise ValueError(f"Unknown prior_guided_calibration={prior_guided_calibration}")

        rho_all = pd.Series(0.5, index=ids_all, dtype=float)
        rho_all.loc[rho_universe.index] = rho_universe.to_numpy(dtype=float)
        prior_resp_all = rho_all.reindex(ids_all).fillna(0.5).to_numpy(dtype=np.float32)
        rho_clip = np.clip(prior_resp_all.astype(np.float64), 1e-6, 1.0 - 1e-6)
        rho_entropy = (
            -(rho_clip * np.log(rho_clip) + (1.0 - rho_clip) * np.log(1.0 - rho_clip))
            / np.log(2.0)
        )
        lower_bound_neg_weight = 1.0 - prior_resp_all - prior_uncertainty_delta * rho_entropy
        prior_neg_weight_all = np.clip(
            lower_bound_neg_weight, prior_weight_floor, 1.0
        ).astype(np.float32)
        print(
            f"{progress_prefix}[EBR-PU] active: calibration={prior_guided_calibration} "
            f"floor={prior_weight_floor:.3f} "
            f"delta={prior_uncertainty_delta:.3f} "
            f"rho_mean={float(np.mean(prior_resp_all[unlabeled_idx])):.4f} "
            f"entropy_mean={float(np.mean(rho_entropy[unlabeled_idx])):.4f} "
            f"neg_weight_mean={float(np.mean(prior_neg_weight_all[unlabeled_idx])):.4f}"
        )
    else:
        print(f"{progress_prefix}[EBR-PU] inactive; using standard nnPU.")

    sup_resp_all = np.full(len(ids_all), 0.5, dtype=np.float32)
    sup_neg_weight_all = np.ones(len(ids_all), dtype=np.float32)
    ...
    if pu_class_prior is None or pu_class_prior <= 0:
        class_prior = float(np.clip(len(train_pos_ids) / max(len(universe_ids), 1), 1e-3, 0.5))
    else:
        class_prior = float(np.clip(pu_class_prior, 1e-3, 0.5))
    ...
    for epoch_idx in range(epochs):
        ...
        # if epoch_idx % sp_update_every == 0:
            ...
            with torch.no_grad():
                ...
                u_sup_scores_list: list[np.ndarray] = []
                for start in range(0, len(unlabeled_idx), batch_size):
                    ...
                    u_sup_scores_list.append(
                        torch.sigmoid(sp_out["sup_logit"]).cpu().numpy()
                    )
            u_scores_all = np.concatenate(u_scores_list)
            u_sup_scores_all = np.concatenate(u_sup_scores_list)
            # v4: keep the full unlabeled pool. Reliability is expressed as a
            # continuous supervised-head negative weight, not as a hard SP-PU mask.
            reliable_neg_mask_u = np.ones_like(u_scores_all, dtype=bool)
            if prior_guided_active:
                sup_resp_u = prior_resp_all[unlabeled_idx].astype(np.float32)
            elif epoch_idx < max(5, warmup_epochs // 2):
                sup_resp_u = np.zeros_like(u_sup_scores_all, dtype=np.float32)
            else:
                sup_resp_u = rank_percentile_scores(u_sup_scores_all)
            sup_resp_all[unlabeled_idx] = sup_resp_u
            sup_neg_weight_all[unlabeled_idx] = np.clip(
            ...
            if epoch_idx % 5 == 0:
                print(
                    ...
                    f"sup_rho_mean={float(np.mean(sup_resp_u)):.4f} "
                    f"sup_neg_w_mean={float(np.mean(sup_neg_weight_all[unlabeled_idx])):.4f}"
                )
            model.train()
        ...
        for _ in range(n_steps):
            ...
            l_sup  = bce(out_l["sup_logit"], y_l)
            l_bce  = bce(out_l["pu_logit"], y_l)
            idx_u_np = idx_u.cpu().numpy()
            if prior_guided_active:
                u_neg_weight = torch.from_numpy(prior_neg_weight_all[idx_u_np]).to(device)
                l_pu = weighted_nnpu_loss(
                    out_p["pu_logit"],
                    out_u["pu_logit"],
                    class_prior,
                    unlabeled_neg_weight=u_neg_weight,
                )
            else:
                u_neg_weight = torch.from_numpy(sup_neg_weight_all[idx_u_np]).to(device)
                l_pu = weighted_nnpu_loss(
                    out_p["pu_logit"],
                    out_u["pu_logit"],
                    class_prior,
            ...
            l_rank = weighted_pairwise_ranking_loss(
                out_p["rank_logit"], out_u["rank_logit"], u_neg_weight
            )
        ...
        with torch.no_grad():
            for idx_v, y_v in val_loader:
                ...
                val_losses.append(float(bce(out_v["sup_logit"], y_v).item()))
                val_sup.append(torch.sigmoid(out_v["sup_logit"]).cpu().numpy())
                val_sc.append(torch.sigmoid(out_v["pu_logit"]).cpu().numpy())
                val_rank.append(torch.sigmoid(out_v["rank_logit"]).cpu().numpy())
        ...
        if val_sc:
            sup_val = np.concatenate(val_sup)
            pu_val = np.concatenate(val_sc)
            rank_val = np.concatenate(val_rank)
            p_val = rrf_fuse_scores(
                sup_val, rrf_fuse_scores(pu_val, rank_val, k=head_rrf_k), k=head_rrf_k
            )
    ...
    with torch.no_grad():
        for start in range(0, len(ids_all), batch_size):
            ...
            all_sup.append(torch.sigmoid(out["sup_logit"]).cpu().numpy())
    ...
    sup_score  = np.concatenate(all_sup)
    ...
    # v4 LSTM-prior path: the neural model contributes the PU disease-risk score.
    # Empirical-prior fusion is applied outside this function by the CV driver.
    deep_score = rrf_fuse_scores(pu_score, rank_score, k=head_rrf_k)
    ...
    info = {
        ...
        "pu_class_prior":    float(class_prior),
        "pu_class_prior_mode": "auto" if pu_class_prior is None or pu_class_prior <= 0 else "fixed",
        "ebr_prior_guided_pu": bool(prior_guided_active),
        "ebr_calibration": str(prior_guided_calibration if prior_guided_active else "none"),
        "ebr_weight_floor": float(prior_weight_floor),
        "ebr_uncertainty_delta": float(prior_uncertainty_delta),
        "ebr_rho_mean_unlabeled": float(np.mean(prior_resp_all[unlabeled_idx])) if len(unlabeled_idx) else float("nan"),
        "ebr_neg_weight_mean_unlabeled": float(np.mean(prior_neg_weight_all[unlabeled_idx])) if len(unlabeled_idx) else float("nan"),
        "sup_rho_mean_unlabeled": float(np.mean(sup_resp_all[unlabeled_idx])) if len(unlabeled_idx) else float("nan"),
        "sup_neg_weight_mean_unlabeled": float(np.mean(sup_neg_weight_all[unlabeled_idx])) if len(unlabeled_idx) else float("nan"),
        "head_rrf_k": int(head_rrf_k),
