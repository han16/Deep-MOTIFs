...
def fit_svm_and_score(
    ...
    """
    Krishnan/Troyanskaya ASD network-prediction style SVM.

    The paper trains an evidence-weighted linear SVM on brain-specific network
    connectivity features and then estimates interpretable probabilities using
    isotonic regression. We keep this project's feature matrix and CV/output
    contract, but use the same estimator family: weighted linear SVM plus
    isotonic calibration of the decision scores.
    """
    ...
    if y_unique.size < 2:
        ...
        return clf, pd.Series(scores, index=X_all.index), {"constant": constant}
    ...
    # Evidence-weighted SVM in the paper assigns a penalty weight to each gene.
    # Our labels do not carry SFARI E1-E4 evidence levels, so we use balanced
    # inverse-frequency sample weights as the available analogue.
    n_pos = int((y_train == 1).sum())
    n_neg = int((y_train == 0).sum())
    w_pos = len(y_train) / max(2 * n_pos, 1)
    w_neg = len(y_train) / max(2 * n_neg, 1)
    sample_weight = np.where(y_train == 1, w_pos, w_neg).astype(float)

    if len(y_train) >= 20 and min(n_pos, n_neg) >= 3:
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=0.2, random_state=random_state + 31
        )
        fit_idx, cal_idx = next(sss.split(X_train, y_train))
    else:
        fit_idx = np.arange(len(y_train))
        cal_idx = np.arange(len(y_train))

    clf.fit(
        X_train.iloc[fit_idx],
        y_train[fit_idx],
        linear_svm__sample_weight=sample_weight[fit_idx],
    )
    cal_decision = clf.decision_function(X_train.iloc[cal_idx])
    y_cal = y_train[cal_idx].astype(int)
    if np.unique(y_cal).size == 2:
        calibrator = IsotonicRegression(out_of_bounds="clip")
        calibrator.fit(cal_decision, y_cal)
        all_decision = clf.decision_function(X_all)
        scores = calibrator.transform(all_decision)
        calibration = "isotonic_holdout"
    else:
        all_decision = clf.decision_function(X_all)
        scores = 1.0 / (1.0 + np.exp(-all_decision))
        calibration = "sigmoid_fallback"

    fit_info = {
        "model": "evidence-weighted linear SVM",
        "kernel": "linear",
        "C": float(c_value),
        "class_weight": "balanced",
        "sample_weight": "balanced_inverse_frequency",
        "probability_calibration": calibration,
        "n_fit": int(len(fit_idx)),
        "n_calibration": int(len(cal_idx)),
    }
    return clf, pd.Series(np.asarray(scores, dtype=float), index=X_all.index), fit_info
...
def run_svm(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_ids, y), start=1):
        ...
        svm_model, final_scores, svm_fit_info = fit_svm_and_score(
            X_train=X_train,
            y_train=y_train,
            X_all=X_all,
        ...
        if hasattr(svm_model, "named_steps") and "linear_svm" in svm_model.named_steps:
            coef = svm_model.named_steps["linear_svm"].coef_.ravel()
            coef_df = pd.DataFrame({
                "feature": X_train.columns,
                "coefficient": coef,
                "abs_coefficient": np.abs(coef),
            }).sort_values("abs_coefficient", ascending=False)
            coef_df.head(500).to_csv(fold_dir / "linear_svm_coefficients_top500.csv", index=False)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Krishnan2016-style evidence-weighted linear SVM baseline."
    )
    ...
    p.add_argument("--output-dir", type=str, default="sv_outputs", help="Output directory name under project root")
...
def main() -> None:
    ...
    print(f"[INFO] Running SVM baseline (string_mode={args.string_mode})...")
    run_svm(
        labels_df=labels_df,
        meta_df=meta_df,
        brainspan_df=brainspan_df,
